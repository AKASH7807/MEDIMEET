...
export default function VideoCall({ sessionId, token }) {
  ...
  const initializeSession = () => {
    ...
    try {
      ...
      sessionRef.current.off("streamCreated").on("streamCreated", (event) => {
        sessionRef.current.subscribe(
          event.stream,
          "subscriber",
          { insertMode: "append", width: "100%", height: "100%" },
          (error) => { if (error) toast.error("Error connecting to other participant's stream"); }
        );
      });

      // Set up session events only once
      sessionRef.current.off("sessionConnected").on("sessionConnected", () => {
        setIsConnected(true);
        setIsLoading(false);
        ...
        publisherRef.current = window.OT.initPublisher(
          ...
          (error) => {
            if (error) {
              ...
              // Publish stream after publisher is ready
      ...
      sessionRef.current.off("sessionDisconnected").on("sessionDisconnected", () => {
      ...
      sessionRef.current.connect(token, (error) => {
        ...
        // Connection success flows to 'sessionConnected'
      });
    } catch (error) {
  ...
  // Render block remains unchanged
